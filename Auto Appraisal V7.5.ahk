...
global KillTask := 0, loopamount := 2, File1 := "CordsSaveForAutoAppraisal.txt", File2 := "MutationSaveForAutoAppraisal.txt", YHalf := "", XHalf := "", HighlightStatus := 0, IsStoppingNoYes := 0
if !(fileexist(File1)) {
	SetOcrRange
	FileAppend
	(
...
if !(FileExist(File2)) {
	GrabMutation
	temporaryvariables := ""
	for i, v in Appraiseable3
	{
		temporaryvariables .= trim(v) "`n"
	}
	fileappend
	(
	"Shiny
...
readthemfile2(fileread(File2))
	...
	/*Appraiseable := ["Sparkling", "Shiny", "Big/Giant",
					"Mythical", "Abyssal", "Fossilized",
					"Lunar", "Midas", "Glossy", "Silver",
					"Mosaic", "Hexed", "Electric", "Darkened",
					"Translucent", "Frozen", "Negative",
					"Scorched", "Albino", "Amber", "Greedy",
					"Poisoned", "Vined", "Crimson", "Spirit",
					"Shrouded"]
	*/
	
MakeTheEntireGUI
thegui.Show("w" otherstarting+180 " h" 20+ytreshhold)
...
global
	...
	if (XHalf or YHalf) {
		if (sumallvar >= 1) {
			...
			msgbox "Please pick something first", "ERROR", "0x40030"
			try winactivate("Roblox")
		}
	}else{
		msgbox "Press `"CTRL + T`" first to set a box to the fish", "ERROR", "0x40030"
		try winactivate("Roblox")
	}
}
...
global
	...
	xpos := xstarting
	ypos := ystarting
	ytreshhold := 300 + ystarting
		for i, v in Appraiseable
		{
			try {
				maps["cb" . v] := thegui.Add("CheckBox", "v" v " x" xpos " y" ypos " h18", v)
				ypos += ydifference
				if (ypos >= ytreshhold) {
					xpos += xdifference
					reachedmax := 1
	...
	otherstarting := xpos + xdifference
;	msgbox otherstarting
}

FindMutation(mutation, MessageNotif := "1") {
global
	if InStr(MutationText, mutation, False) {
		if (MessageNotif != 0) {
			msgbox mutation " is Found", "Success", "0x40000"
		}
		sleep 10
		Highlight()
		Highlight2()
		maps["cb" mutation].value := !maps["cb" mutation].value
		global KillTask := 1
	}
}
...
global
	loop loopamount {
		MutationText := OCR.FromRect(xxxxxxxx, yyyyyyyy, wwwwwwww, hhhhhhhh, "en-us",2).Text
		for k, v in maps
		{
			if (k != "cbBig/Giant") {
				if (v.value == 1) {
					FindMutation(strreplace(k, "cb", ""))
				}
			}else{
				if (v.value == 1) {
					FindBigOrGiant()
...
global
	loop loopamount {
		if pixelsearch(&cc, &kk, Left, bottom, Right, Top, 0x87ED86, 5) {
			msgbox "Big/Giant is Found", "Success", "0x40000"
			sleep 10
			Highlight()
			Highlight2()
			maps["cbBig/Giant"].value := !maps["cbBig/Giant"].value
			global KillTask := 1
		}
		if (KillTask = 1) {
			break
...
global
	...
	MsgBox("Click `"ok`" after you set box in the Text", "Setting", "0x40000")
...
global
	...
	WinSetTransparent(90)
;	MsgBox("Click `"ok`" after you set box in the Text", "Setting", "0x40000")
	TheFatRetakeBoxUI := (*) => ttp("Click `"R`" when your done setting the box`n`n(Set it to the appraisal notification)`nmake it as big as possible`nbut dont cover the toolbar`nand while running dont cover the macro GUI",,,1)
...
timeout := 0
loop {
if (pixelsearch(&template, &template, thenoclickx-40, thenoclicky-20, thenoclickx+40, thenoclicky+20, 0xD43D00, tolerance2332432) and pixelsearch(&template, &template, theyesclickx-40, theyesclicky-20, theyesclickx+40, theyesclicky+20, 0xA8FF95, tolerance2332432)) {
break
}
if (timeout >= 100) {
break
...
GetHTML(url) {
	...
	req.Option[4] := 13056  ; Ignore SSL errors (optional)
    req.Option[6] := false  ; Disable auto-redirect
    req.Option[9] := false  ; Disable proxy auto-detect
    req.Send()
...
global
	var1234 := ""
	loop parse, GetHTML("https://fischipedia.org/wiki/Appraiser"), "`n"
	{
		if (instr(A_LoopField, "<td><span class=`"mutation mutation")) {
			if RegExMatch(A_LoopField, ">([^<]+)</a>", &LeMutation) {
				var1234 .= trim(LeMutation[1]) ","
			}
		}
	}
	if (trim(var1234) == "") {
		var1234 := ""
		loop parse, GetHTML("https://raw.githubusercontent.com/lolzzn/Auto-Appraisal/refs/heads/main/raw%20for%20mutation%20incase%20it%20fail"), "`n"
		{
			var1234 .= trim(A_LoopField) ","
		}
	}
	Appraiseable2 := strsplit(var1234, ",")
	Appraiseable3 := []
	for i, v in Appraiseable2
	{
		if (trim(v) != "" and trim(v) != "Shiny" and trim(v) != "Sparkling") {
			Appraiseable3.push(trim(v))
		}
	}
}
...
UpdateMutation() {
global
	result := msgbox("Are you sure? This process might take a while", "Update Mutation", "0x40004")
	if (result == "No") {
		return
	}
	GrabMutation
	...
	thegui.destroy
	MakeTheEntireGUI
	thegui.Show("x" Xx2 " y" Yy2 " w" otherstarting+180 " h" 20+ytreshhold)
}

TakeAppraiseAnywhereSettings() {
global
	fatarrowsettimer := (*) => ttp("Press `"R`" on `"Appraise Fish`"",,, 1)
	...
	ttp ,,, 1
	fatarrowsettimer := (*) => ttp("Press `"R`" on the middle of `"Okay`"`n(appraise without holding a fish)",,, 1)
	settimer fatarrowsettimer, 150
	keywait "R", "D"
	mousegetpos &thenofishconfirmationx, &thenofishconfirmationy
	keywait "R"
	settimer fatarrowsettimer, 0
	ttp ,,, 1
	fatarrowsettimer := (*) => ttp("Press `"R`" on the middle of `"Cancel`"`n(appraise while holding a fish)",,, 1)
	settimer fatarrowsettimer, 150
	keywait "R", "D"
	...
	settimer fatarrowsettimer, 0
	ttp ,,, 1
	fatarrowsettimer := (*) => ttp("Press `"R`" on the middle of `"Confirm`"`n(appraise while holding a fish)",,, 1)
	settimer fatarrowsettimer, 150
	keywait "R", "D"
	...
	UpdateFile1Thing()
	readthemfile(fileread(File1))
	msgbox "Settings have been succesfully saved`nValues:`nAppraise Anywhere:" theappraiseanywherex ", " theappraiseanywherey "`nOkay Button: " thenofishconfirmationx ", " thenofishconfirmationy "`nCancel Button: " thenoclickx ", " thenoclicky "`nConfirm Button: " theyesclickx ", " theyesclicky, "Settings Saved", "0x40040"
	try winactivate("Roblox")
}
...
global
	...
	thegui.SetFont("s10 cwhite", "Tahoma")
	MakeGUI()
	AppraiseAnywhereOption := thegui.Add("Checkbox", "x" otherstarting+20 " y120 +background3b3b3b w150", "Appraise Anywhere")
	...
	thegui.Add("Text", "x" otherstarting+0 " y10 w200", "Auto Appraisal V7.5")
	...
	thegui.Add("Text", "x" otherstarting-7 " y65 w250", "`"Ctrl + Q`" to Start `n`"Ctrl + T`" Set Box To Fish `n`"Alt`" To Force Stop")
	heightofgroupbox := (reachedmax) ? (ytreshhold-ystarting)+16 : ypos+8
	thegui.Add("groupbox", "x" xstarting-8 " y" ystarting-13 " w" otherstarting-xstarting-8 " h" heightofgroupbox)
	thegui.SetFont("s10 cwhite", "Tahoma")
	TakeAppraisalSettings := thegui.add("button", "x" otherstarting+20 " y139 w125", "Change ^^ Settings")
	TakeAppraisalSettings.OnEvent("Click", (*) => TakeAppraiseAnywhereSettings())
	ToggleRedBoxThing := thegui.add("button", "x" otherstarting+20 " y173 w125", "Toggle Highlight")
	ToggleRedBoxThing.OnEvent("Click", (*) => ToggleHighlight())
	AutoUpdateMutation := thegui.add("button", "x" otherstarting+20 " y207 w125", "Update Mutation")
	AutoUpdateMutation.OnEvent("Click", (*) => UpdateMutation())
	uncheckallboxes := thegui.add("button", "x" otherstarting+20 " y240 w60", "Uncheck")
	uncheckallboxes.OnEvent("Click", (*) => dosomethingtoboxes(0))
	checkallboxes := thegui.add("button", "x" otherstarting+85 " y240 w60", "Check")
	checkallboxes.OnEvent("Click", (*) => dosomethingtoboxes(1))
	thegui.SetFont("s13 cwhite", "Tahoma")
	retakeboxsettings := thegui.add("button", "x" otherstarting " y275 w165", "Retake Box Settings")
	retakeboxsettings.OnEvent("Click", takeocrbox)

	thegui.Title := "Auto Appraisal v7.5"
