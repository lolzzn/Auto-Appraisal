...
global KillTask := 0, loopamount := 2, stopallcoderunning := 0, File1 := "CordsSaveForAutoAppraisal.txt", File2 := "MutationSaveForAutoAppraisal.txt", YHalf := "", XHalf := ""
if !(fileexist(File1)) {
	SetOcrRange
	FileAppend
	(
	"xxxxxxxx=" WindowLeft "`n"
	"yyyyyyyy=" WindowTop "`n"
	"wwwwwwww=" WindowWidth "`n"
	"hhhhhhhh=" WindowHeight "`n"
	"sw=2000 `n"
	), File1
	sleep 750
}
...
global
	...
	WinSetTransparent(90)
	MsgBox("Click `"ok`" after you set box in the Text", "Setting", "0x40000")
	WinGetPos(&WindowLeft, &WindowTop, &WindowWidth, &WindowHeight, ScaleGUI)
	ScaleGUI.Hide()
}
...
global
	...
	MakeGUI()
	sleepwait := thegui.Add("edit", "x" otherstarting+33 " y155 +background3b3b3b w100", sw)
	sleepwait.OnEvent("change", sleepdelayupdate)
	SendMessage(0x1501, false, StrPtr("Sleep Delay"), sleepwait.Hwnd)
	thegui.SetFont("s11 cwhite", "Tahoma")
	thegui.Add("Text", "x" otherstarting+45 " y135 w200", "Sleep delay")
	...
	thegui.Add("Text", "x" otherstarting-10 " y65 w250", " `"Ctrl + Q`" to Start `n `"F1`" to Exit | `"F2`" to Reload `n `"Ctrl + T`" Set Box To Fish `n `"Alt`" To Force Stop")
	...
	thegui.SetFont("s10 cwhite", "Tahoma")
	AutoUpdateMutation := thegui.add("button", "x" otherstarting+20 " y190 w125", "Update Mutation")
	AutoUpdateMutation.OnEvent("Click", (*) => UpdateMutation())
	uncheckallboxes := thegui.add("button", "x" otherstarting+20 " y230 w60", "Uncheck")
	uncheckallboxes.OnEvent("Click", (*) => dosomethingtoboxes(0))
	checkallboxes := thegui.add("button", "x" otherstarting+85 " y230 w60", "Check")
